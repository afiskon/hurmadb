...
PgsqlWorker::PgsqlWorker(Socket& socket, PersistentStorage* storage): _socket(socket){
...
void PgsqlWorker::sendSelectQueryResult(int16_t num_of_columns, int16_t num_of_rows, vector<DataColumn> columns, deque<vector<string>> rows){
    ...
    sendSizeOfBlock(sizeof(uint32_t) +
...
PgsqlServer::PgsqlServer(PersistentStorage* storage): TcpServer(&PgsqlWorkerThreadProc) {
    this->storage = storage;
}
